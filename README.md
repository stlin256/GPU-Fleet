...
Agent 更新是独立的 pull 模型：设置页只保存目标版本、签名 manifest URL、Ed25519 公钥、检查间隔和并发上限；Agent 通过 HMAC 拉取策略，自行下载 manifest 和 artifact，验证签名与 sha256 后只替换自己的二进制，并把检查、失败、应用或回滚事件上报给服务端审计。服务端不会向 Agent 下发 shell 命令。
