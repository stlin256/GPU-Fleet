...
- Web 面板支持登录、退出、卡片式多机多卡 GPU Fleet 总览、设备管理、GPU 当前状态、GPU 进程快照、24 小时统计、深浅主题切换和服务端边界展示。
...
- 已提供 Chrome/Edge CDP 前端验证脚本，覆盖登录、刷新会话恢复、GPU Fleet 卡片面板、深浅主题切换和持久化、设备页、移动端总览、移动端 GPU 页、截图和移动端横向溢出检查；演示验证可强制检查 5 张 GPU 卡片、离线蒙版和同设备双卡。
